...
import { wellgorize } from "../packages/wellgorize";

function ForYouPage({ profile = {}, isExplore }) {
  const [publications, setPublications] = useState([]);

  const posts = [...publications];

  async function wellgorizeMyPost() {
    try {
      wellgorize(posts);
    } catch (e) {
      console.error("cannot wellgorize", e);
    }
  }
  wellgorizeMyPost();

  const [getTimeline, timelineData] = useLazyQuery(GET_TIMELINE);
  const [explorePublications, explorePublicationsData] =
    useLazyQuery(EXPLORE_PUBLICATIONS);
